...
import { changeTitle } from '../../store/features/newTitleSlice'
...
const CardDeckList: FC = () => {
  ...
  const deleteDeck = (id: number): void => {
    dispatch(removeDeck(id))
  }
  //  Значение функции titleDeck
  const titleDeck = (newValue: string): string => {
  ...
  return (
    <section>
      <Container>
        <div className='grid grid-cols-4 gap-3 mt-5 max-h-96 overflow-y-auto lg:grid-cols-2 xs:grid-cols-1'>
          {deckList.map((item, idx) => (
            <div key={idx} className='border-2 p-2 animate-fade-in flex justify-between border-gray-400 rounded-md'>
              <Link className='transition-colors duration-300 ease-linear overflow-x-auto hover:text-gray-400' to={`/deck/${item.id}`} onClick={() => titleDeck(item.title)}>
                {item.title}
              </Link>
