...
const ModalWord = () => {
  ...
  return (
    <section>
      <div onClick={ModalTaskOpen} className="fixed inset-0 bg-gray-800 bg-opacity-75 z-50"></div>
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 bg-white p-5 h-72 rounded-md w-1/2 md:w-4/5 xs:w-[90%] xxs:w-[95%]">
        <button onClick={ModalTaskOpen} className='block ml-auto transition duration-300 ease-linear hover:bg-red-500'>
        ...
        <Button className='bg-mainColor px-4 py-2 mt-12 rounded-md text-white block mx-auto transition-colors duration-300 ease-in hover:bg-green-700' >
          Добавить карту
        </Button>
      </div>
    </section>
